eye=io_loadavi('G:\tmp\05_bi30\hql083\250719_hql083_sleep\HQL083_sleep_250719_002\eye.avi');
%%
...
imageHeight=size(eye,1); %How many pixels tall is the frame
imageWidth=size(eye,2);%How many pixels wide is the frame
pixelsPerFrame = imageWidth*imageHeight;
skippedPixels = pixelsPerFrame; 
%% Empty Structures
Pupil_Area(1:size(eye,3))=NaN; %Area of pupil
Pupil_Major(1:size(eye,3))=NaN;% Length of major axis of pupil
Pupil_Minor(1:size(eye,3))=NaN;% Length of minor axis of pupil
Pupil_Centroid(1:size(eye,3),2)=NaN; %Center of pupil
PupilBoundary(1:size(eye,1),1:size(eye,2),1:size(eye,3))=NaN;
PupilPix=cell(1,size(eye,3));
...
WorkingImg=imcomplement(eye(:,:,2)); %grab frame from image stack
if isempty(BW) %If no predefined ROI was fed to function
...
ThreshImg=double(double(FiltImg).*BW); %Only look at pixel values in ROI
...
testThresh=labeloverlay(eye(:,:,1),testImg);
...
imageFrames=gpuArray(eye);
roiInt(1:size(imageFrames,3))=NaN;
roiInt=gpuArray(roiInt);
for framenum=1:size(eye,3)
    disp(framenum)
    FiltImg=medfilt2(imcomplement(imageFrames(:,:,framenum)),medFilt_Params);
    ...
    [h, w] = size(eye(:,:,framenum));
    RadonCropped = RadonPupil(1:h, 1:w);
    FillPupilCropped = FillPupil(1:h, 1:w);
    theArea = [];
    ...
    if ~isempty(area_filled)
        ...
        Hold = labeloverlay(eye(:,:,framenum), FillPupilCropped, 'Transparency', 0.8);
        ...
        PupilBoundary(:,:,framenum)=FillPupil;
        Hold=labeloverlay(eye(:,:,framenum),FillPupil);
...
%% Determine when eyes are closed.
...
PupilTime=(1:length(PupilTracker.Pupil_Area))/cameraFs;
